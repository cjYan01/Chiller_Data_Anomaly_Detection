...
def detect_kmeans_anomaly_rowwise(df, feature_cols, n_clusters=3, contamination=0.05, random_state=42):
    ...
    anomaly_indices = data.index[distances > threshold]
    anomaly = pd.Series(False, index=df.index)
...
def main():
    ...
    with st.sidebar.expander("🤖 机器学习方法", expanded=False):
        use_kmeans = st.checkbox("K-means聚类", value=False)
        if use_kmeans:
            numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
            default_cols = [col for col in numeric_cols if "status" not in col]
            selected_features = st.multiselect("选择特征列", options=numeric_cols, default=default_cols[:10])
            n_clusters = st.number_input("簇数 K", min_value=2, value=3, step=1)
            contamination = st.slider("异常比例", min_value=0.01, max_value=0.5, value=0.05, step=0.01)
            if selected_features:
                methods_to_run.append(("kmeans", {"features": selected_features, "n_clusters": n_clusters, "contamination": contamination}, False))
    ...
    if run_button:
        ...
        for method_name, params, is_colwise in methods_to_run:
            ...
            if method_name == "physical":
                ...
                row_mask = detect_kmeans_anomaly_rowwise(df, features, n_clusters, contamination)
